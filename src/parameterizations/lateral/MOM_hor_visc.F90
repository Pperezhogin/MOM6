...
subroutine horizontal_viscosity(u, v, h, uh, vh, diffu, diffv, MEKE, VarMix, G, GV, US, &
  ...
  !$OMP   KH_u_GME, KH_v_GME, grid_Re_Kh, grid_Re_Ah, NoSt, ShSt, hu_cont, hv_cont, STOCH, dt &
  !$OMP ) &
  !$OMP private( &
  !$OMP   i, j, k, n, tmp, &
  ...
  do k=1,nz
    ...
    if (CS%use_ZB2020) then
      call ZB2020_copy_gradient_and_thickness(sh_xx, sh_xy, vort_xy, hq, G, GV, CS%ZB2020, k, dt)
    endif
